...
init_fade (NemoDesktopBackground *self)
{
    ...
    /* Currently the background fade effect doesn't work properly
     * Just set this to always false since it give a quicker
     * background change
     */
                   ...
                   gpointer user_data)
{
    ...
    self->details->screen_size_handler = 
        g_signal_connect (screen, "size-changed", G_CALLBACK (screen_size_changed), self);
